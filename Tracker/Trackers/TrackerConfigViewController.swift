...
final class TrackerConfigViewController: UIViewController, UICollectionViewDelegateFlowLayout {
    ...
    func configureCollectionView() {
        ...
        readyButton = Button(type: .primary(isActive: false), title: "Готово", tapHandler: {
            self.delegate?.didCreateNewTracker(newCategory: self.createNewTracker() as! TrackerCategory)
...
extension TrackerConfigViewController {
    ...
    func isTrackerReadyToBeCreated() {
        if let readyButton = readyButton {
            readyButton.addTarget(self, action: #selector(createNewTracker), for: .touchUpInside)
            if let title = selectedTitle, !title.isEmpty,
                let emoji = selectedEmoji,
                let color = selectedColor,
                !selectedShedule.isEmpty {
...
//    }
    ...
    //    private func isTrackerReadyToBeCreated() {
    //        if let title = selectedTitle, !title.isEmpty,
    //            let emoji = selectedEmoji,
    //            let color = selectedColor,
    //            !selectedShedule.isEmpty {
    ...
    @objc func createNewTracker() -> Any {
